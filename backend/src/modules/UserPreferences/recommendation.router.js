...
recommendationRouter.get('/:userId', async (req, res) => {
  ...
  const userVectors = preferences.interactedProductIds.map(vectorizeProduct);
  const userProfile = {};
  ...
  const scoredProducts = allProducts
    .filter(p => !preferences.interactedProductIds.some((ip) => ip._id.equals(p._id)))
    .map(product => {
  ...
  res.json(scoredProducts.map(p => p.product));
});
